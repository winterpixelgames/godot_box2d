...
void Box2DFixture::create_b2Fixture(b2Fixture *&p_fixture_out, const b2FixtureDef &p_def, const Transform2D &p_shape_xform) {
	...
	switch (p_def.shape->m_type) {
		...
		case b2Shape::Type::e_sdf: {
			b2SDFShape shp = b2SDFShape(*static_cast<const b2SDFShape *>(p_def.shape));
			shp.m_p = gd_to_b2(p_shape_xform.xform(b2_to_gd(shp.m_p)));
			finalDef.shape = &shp;
			p_fixture_out = owner_node->body->CreateFixture(&finalDef); // Write here because shp is in scope
		} break;
...
void Box2DFixture::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("get_collision_object"), &Box2DFixture::get_owner);
...
void Box2DFixture::set_shape(const Ref<Box2DShape> &p_shape) {
	...
	Box2DSDFShape* sdfShape = Object::cast_to<Box2DSDFShape>(p);
	if(sdfShape) {
