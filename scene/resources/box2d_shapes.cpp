...
void Box2DPolygonShape::build_polygon() {
	...
	switch (build_mode) {
		...
		case Box2DPolygonShape::BUILD_OPEN_SEGMENTS: {
			ERR_FAIL_COND_MSG(points.size() < 4, "Open segment polygon must have at least 4 points.");
...
void Box2DSDFShape::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("set_map_func"), &Box2DSDFShape::set_map_func);
...
void Box2DSDFShape::get_map_func() const {
	// do nothing
}
